...
#This was login serializer
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):
    @classmethod
    def get_token(cls, user):
        token = super().get_token(user)
        #This code below is customizing the token & adding extra information to it
...
class RegisterSerializer(serializers.ModelSerializer):
    #Now we need to define fields for the serializer which will include password1 and password2 field
    password = serializers.CharField(write_only = True, required = True, validators = [validate_password])
    #You don't need validators again because password1 will be same as password2
    password2 = serializers.CharField(write_only = True, required = True)

    class Meta:
        model = User
        #Fields that you wanna add in registration form
    ...
    #A method to create user based on validated data
    def create(self, validated_data):
        ...
        user.save()
        #Returning user that just got created
        return user
