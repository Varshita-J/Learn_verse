...
class RegisterView(generics.CreateAPIView):
    ...
    #AllowAny means any user both authenticated and unauthenticated can access this view
    permission_classes = [AllowAny]
    #All API Views must finally include the serializer class(Why so?)
    serializer_class = api_serializer.RegisterSerializer


def generate_random_otp(length = 7):
    #Now let's go ahead & generate a random 7-digit OTP
    otp = ''.join([str(random.randint(0, 9)) for _ in range(length)])
    return otp

#If it's retrieve API View, that means we are trying to get one single item
#If it's a list API View, it means we are trying to get multiple items, which pretty much means we are trying to get query set
class PasswordResetEmailVerifyAPIView(generics.RetrieveAPIView):
    permission_classes = [AllowAny]
    #It's User serializer because what we want to update will be user's password
    #So we have to interact with the User model & things like that
    serializer_class = api_serializer.UserSerializer

    #Let's overwrite the defalut get_object method.. Wait what is the defalut method?
    def get_object(self):
        ...
        if user:
            ...
            #uuidb64 is like user id we use to verify or identify the user
            uuidb64 = user.pk
            ...
            #Now we want to go ahead and generate a link that will be sent to user's email
...
#Remember, I told createAPIView is used for pretty much creatinga  new object
class PasswordChangeAPIView(generics.CreateAPIView):
    permission_classes = [AllowAny]
    serializer_class = api_serializer.UserSerializer

    def create(self, request, *args, **kwargs):
        #After all that we will be sending a data in the payload back to this view from the front end
        payload = request.data
        #Now in the payload, we will be sending the OTP, the uuid64, and the new password user wants to use
        otp = payload['otp']
...
class CreateOrderAPIView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # print("Hi")
        print("Request from views.py: ", request)
